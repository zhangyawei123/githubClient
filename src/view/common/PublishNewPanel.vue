  ...
  <div class="panel-body banner-panel">
    ...
    <div class="footer clearfix">
      ...
      <div class="oprate-box">
        ...
        <a href="javascript:void(0);" class="cancel-btn" @click="closeDialog">取消</a>
      </div>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      posterSelectionChange(val,row) {
        this.$refs.equipmentTable.clearSelection();
        this.ChooseList = val;
        this.ChooseList[0].showType = '海报';
        if(val.length ===0) {
          return
        }
        if (val.length >1) {            //海报选项超过一个
          this.$notify.error({
            title: '错误',
            message: '最多添加1条海报'
          });
      ...
      equipmentSelectionChange(val,row) {
        this.$refs.posterTable.clearSelection();
        this.ChooseList = val;
        this.ChooseList[0].showType = '设备';
        if(val.length ===0) {
          return
        }
        if (val.length >1) {            //海报选项超过一个
      ...
      sendBannerData() {
        ...
        if(this.ChooseList.length ===0) {
          this.$notify.error({
            title: '错误',
            message: '请至少选中一条海报或者产品'
          });
          return
        }
        this.$emit('sendBannerData', this.ChooseList[0]);
        // 传递数据后清空选择
        this.ChooseList = [];
        this.$refs.posterTable.clearSelection();
        this.$refs.equipmentTable.clearSelection();
