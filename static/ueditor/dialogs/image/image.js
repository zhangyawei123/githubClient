    ...
    SearchImage.prototype = {
        ...
        initEvents: function(){
            ...
            domUtils.on($G('searchBtn'), 'click', function(){
                ...
                if(key && key != lang.searchRemind) {
                	SearchImage.prototype.pagenum = 1;
            ...
            domUtils.on($G('searchList'), 'click', function(e){
                ...
                if (li.tagName.toLowerCase() == 'li') {
                	$("#searchListUl").children("li").removeClass("selected");
        ...
        getImageData: function(){
            var _this = this,
                ...
                url = "https://mp.toutiao.com/article/get_ic_picture_list/?term="+key+"&pagenum="+(SearchImage.prototype.pagenum++) +"&page_size=32";
            ajax.request(url, {
                'dataType': 'jsonp',
                'onsuccess':function(json){
                    var list = [];
                    if(json && json.data) {
                    	if(SearchImage.prototype.pagenum == 2){
                    		$G('searchListUl').innerHTML = "";
                    	}
                        for(var i = 0; i < json.data.list.length; i++) {
                        	var ima = json.data.list[i];
                            if(ima.img) {
                                list.push({
                                	grapher_name: ima.grapher_name,
                                    img: ima.img,
                                    small_img: ima.small_img,
                                    id: ima.id
                                });
                            }
                        }
        ...
        setList: function (list) {
            ...
            if(list.length) {
                for (i = 0; i < list.length; i++) {
                    ...
                    img.width = 113;
                    img.setAttribute('src', list[i].small_img);
                    img.setAttribute('big_src', list[i].img);
        ...
        getInsertList: function () {
            ...
            for(var i = 0; i < items.length; i++) {
                ...
                if(child.tagName && child.tagName.toLowerCase() == 'img' && domUtils.hasClass(items[i], 'selected')) {
                    src = child.src;
                    _src = child.getAttribute("big_src");
                    list.push({
                        src: src,
                        _src: _src,
